...
def main():
    ...
    if os.path.isdir(training_args.output_dir) and not training_args.overwrite_output_dir: # if there is a prior output directory, and do_train = True, and overwrite_output_dir = False, then it tries to find last checkpoint saved in the directory
    ...
    data_files = {"train": '../data/clean/' + data_args.train_file, "validation": '../data/clean/' + data_args.validation_file, "test": '../data/clean/' + data_args.test_file} # file paths to train/validation/test data, `data_files` is a dictionary with keys    
    
    for key in data_files.keys(): # logs file paths for "train", "validation", and "test" in `data_files`
    ...
    study.optimize(lambda trial: objective(trial, data_args, training_args, logger, tokenizer, last_checkpoint, preprocess_function, compute_metrics, model, data_collator, get_search_space, clean_datasets), 
                   n_trials = training_args.n_trials)
    
    # Create a string representation of the best hyperparameters with reduced decimals for floats
    best_param_str = ", ".join(
    ...
    # combine hyperparameter optimization results into a dictionary
    optimized_hyperparams = {
        'best_hyperparameters': study.best_params,
        'best_accuracy': study.best_value
    }
    
    # save hyperparameter optimization results as a json file
    with open('../results/hyperparameter_optimization/optimized_hyperparameters.json', 'w') as f:
