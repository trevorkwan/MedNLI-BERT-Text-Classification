...
# from transformers.utils import send_example_telemetry # optional function that sends data to transformers team
...
def main():
    ...
    metrics = best_trainer.evaluate(eval_dataset=test_dataset) # we loaded the metric to be accuracy
    metrics["test_samples"] = len(test_dataset) # put # of valid samples as key in metrics dict
    # e.g. {"accuracy":0.85, "test_samples":500}

    best_trainer.log_metrics("test", metrics) # "eval" is string message, logs the `metrics`
    best_trainer.save_metrics("test", metrics) # "eval" is the string message, saves `metrics` to disk in a file named `metrics.json` in the output directory
